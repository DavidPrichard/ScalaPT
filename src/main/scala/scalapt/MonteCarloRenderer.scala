...
class MonteCarloRenderer(
  ...
  override def radiance(ray: Ray, depth: Int): RNG.Type[RGB] = {
          ...
          if (newDepth > 5) {
            ...
            RNG.nextDouble.flatMap(rnd => {
              if (rnd >= max) {
                ...
                prim.material.radiance(this, ray, newDepth, isect, n, nl).map(_ * colour / max)
              }
            })
          } else {
            prim.material.radiance(this, ray, newDepth, isect, n, nl).map(_ * colour)
          }
        }
